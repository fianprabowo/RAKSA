...
const ROWS: { key: string; label: string; dot: string; bar: string }[] = [
  { key: WristbandStatus.ACTIVE, label: "Aktif", dot: "bg-emerald-500", bar: "bg-emerald-500" },
  { key: WristbandStatus.CLAIMED, label: "Perlu setup", dot: "bg-amber-500", bar: "bg-amber-500" },
  { key: WristbandStatus.DISABLED, label: "Nonaktif", dot: "bg-slate-400", bar: "bg-slate-400" },
  { key: WristbandStatus.REVOKED, label: "Dicabut", dot: "bg-red-500", bar: "bg-red-500" },
];
...
export function StatusBreakdown({ wristbands }: StatusBreakdownProps) {
  ...
  return (
    <div className="flex h-full flex-col rounded-[var(--radius-card)] border border-slate-200/80 bg-white p-5 shadow-[var(--shadow-soft)] dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <h2 className="text-base font-semibold tracking-tight text-slate-900 dark:text-slate-100">Status Tag</h2>
      <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Distribusi seluruh tag keluarga</p>

      {total === 0 ? (
        <div className="flex flex-1 items-center justify-center py-8">
          <p className="text-sm text-slate-400 dark:text-slate-500">Belum ada tag terdaftar</p>
        </div>
      ) : (
